...
function setupWorker(server, matchMaker) {
    ...
    process.on('message', function (message, socket) {
        ...
        if (message[0] === Protocol_1.Protocol.PASS_HTTP_SOCKET) {
            ...
            var roomNameOrId_1 = message[1];
            var clientOptions = message[2];
            var room = void 0, roomId_1, score_1 = 1;
            if (_1.isValidId(roomNameOrId_1)) {
                room = matchMaker.joinById(roomNameOrId_1, clientOptions);
            }
            else {
                var result = matchMaker.getAvailableRoomByScore(roomNameOrId_1, clientOptions);
                room = result.room;
                score_1 = result.score;
            }
            roomId_1 = room && room.roomId;
